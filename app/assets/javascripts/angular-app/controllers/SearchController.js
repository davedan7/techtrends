  ...
  $scope.submitSearch = function() {     // Is there a way to refactor out the chain of promises?
    for (var i = 0; i < $scope.developerSearchWords.length; i++) {
      ...
      .success(function(data){
        ...
        // $scope.descriptionResults = descriptionParse(data);
        for (var i = 0; i < $scope.rawData.length; i++) {
          titleParse($scope.rawData[i]);
          descriptionParse($scope.rawData[i]);
    ...
    var chart = c3.generate({
      data: {
        bindto: '.chart',
          ...
          onclick: function (d) { generateDescriptionChart(d.id);},
